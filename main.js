...
if (!gotTheLock) {
    ...
    async function applyTheme(theme) {
        ...
        if (themeToApply === 'auto') nativeTheme.themeSource = 'system';
        else if (themeToApply === 'light') nativeTheme.themeSource = 'light';
        else if (themeToApply === 'dark') nativeTheme.themeSource = 'dark';
        await settings.set('theme', themeToApply);
    }

    async function initializeTheme() {
        const savedTheme = await settings.get('theme');
        await applyTheme(savedTheme || 'auto');
    }
    ...
    function setupAutoUpdater() {
        ...
        autoUpdater.on('update-downloaded', () => {
            new Notification({ title: 'Update Ready', body: 'Restart now to install the update.' }).show();
